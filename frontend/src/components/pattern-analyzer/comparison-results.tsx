...
export default function ComparisonResults({ results, originalCode }: ComparisonResultsProps) {
  const [selectedResult, setSelectedResult] = useState<string>(results[0]?.path || "");

  // Find the currently selected result
  const currentResult = results.find((r) => r.path === selectedResult) || results[0];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <Card className="p-4 lg:col-span-1">
        <h3 className="text-lg font-semibold mb-4">Similar Implementations</h3>
            ...
            {results.map((result) => (
              <Card
                ...
                className={`p-4 cursor-pointer transition-colors hover:bg-accent/50 ${
                  selectedResult === result.path ? "border-primary/50 bg-accent" : ""
                }`}
                onClick={() => setSelectedResult(result.path)}
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="font-medium text-base truncate" title={result.name}>
                      {result.name}
                    </h4>
                  ...
                  <Badge variant={result.analysis.similarityScore > 70 ? "default" : "outline"}>
          ...
          <>
            <Card className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold">{currentResult.repository.name}</h3>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Badge variant={currentResult.analysis.similarityScore > 70 ? "default" : "secondary"}>
              ...
              <Tabs defaultValue="comparison">
                <TabsList className="mb-4">
                  ...
                  <TabsTrigger value="insights">Implementation Insights</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="insights" className="mt-0">
                  <Card className="p-4 bg-accent/30">
                    ...
                    <div>
                      <h4 className="font-semibold text-base">Key Differences</h4>
                      <ul className="mt-2 text-sm space-y-2">
                        {currentResult.analysis.highlightedDifferences.map((diff, index) => (
                    ...
                    <div className="mt-6 bg-blue-50 dark:bg-blue-950 p-4 rounded-md">
                      <h4 className="font-semibold text-base">Implementation Quality</h4>
                      <div className="flex items-center mt-2">
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                          <div 
                            className="bg-primary h-2.5 rounded-full" 
                            style={{ width: `${currentResult.analysis.similarityScore}%` }}
                          ></div>
                        </div>
            ...
            <Card className="p-4">
              <h3 className="text-lg font-semibold mb-2">Repository Details</h3>
              <p className="text-sm mb-4">{currentResult.repository.description}</p>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-500">Stars</h4>
                  <p className="font-mono">{currentResult.repository.stars.toLocaleString()}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500">Forks</h4>
                  <p className="font-mono">{currentResult.repository.forks.toLocaleString()}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-500">Language</h4>
                  <p>{currentResult.repository.language || "Unknown"}</p>
                </div>
                <div>
